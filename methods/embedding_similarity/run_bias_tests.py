"""
Bias robustness tests for Method 2a (Embedding Similarity).
...
Output:
  results/bias_format_results_2a.json
  results/bias_name_results_2a.json
  results/bias_format_summary_2a.csv
  results/bias_name_summary_2a.csv
...
BASELINE_PATH = RESULTS_DIR / "embedding_results.json"
...
def run_variants(pairs, variant_map, pdf_dir, jd_texts, baseline, save_path, label, matcher):
    results = {}
    if save_path.exists():
    ...
    for pair_id in pairs:
        ...
        for variant_name, suffix in variant_map.items():
            ...
            try:
                ...
                res = matcher.match(cv_text, jd_texts[pair_id], section_aware=True)
...
def main():
    ...
    fmt_results = run_variants(
        ...
        RESULTS_DIR / "bias_format_results_2a.json", "FORMAT BIAS (2a)", matcher,
    )
    fmt_rows = write_summary_csv(
        fmt_results, RESULTS_DIR / "bias_format_summary_2a.csv", list(FORMAT_VARIANTS.keys())
    )
    ...
    name_results = run_variants(
        ...
        RESULTS_DIR / "bias_name_results_2a.json", "NAME BIAS (2a)", matcher,
    )
    name_rows = write_summary_csv(
        name_results, RESULTS_DIR / "bias_name_summary_2a.csv", NAME_VARIANTS
    )

    print("\n==============================")
    print("FORMAT BIAS SUMMARY (2a)")
    print("==============================")
